<?php
/**
 * admin/index.php — Blocks overview (each block: 1 teacher adviser, many students;
 * a teacher may handle multiple blocks).
...
<div class="page-head">
  ...
  <div class="actions">
    <button type="button" class="btn" id="btn-create-block" data-testid="open-create-block">Create block</button>
  </div>
</div>
...
<template id="tpl-create-block">
  <form method="post" action="actions.php" id="create-block-form" data-testid="add-block-form">
    <?= csrf_field() ?>
    <input type="hidden" name="action" value="add_block">
    <div class="form-grid">
      <div class="field">
        <label>Block name</label>
        <input class="input" name="name" placeholder="e.g. BSIT-Block1" data-testid="block-name" required data-autofocus>
      </div>
      <div class="field">
    ...
    <div data-modal-footer>
      ...
      <button class="btn" type="submit" data-testid="block-submit">Create block</button>
    </div>
  </form>
</template>
...
<?php if (!$blocks): ?>
  <p class="empty"><?= $q !== '' ? 'No blocks match your search.' : 'No blocks yet — use Create block to add one.' ?></p>
<?php endif; ?>
<script>
document.addEventListener('DOMContentLoaded', function () {
  ...
  function wireAcademicsForms() {
    document.querySelectorAll('.block-academics-form').forEach(function (form) {
  ...
  var openBtn = document.getElementById('btn-create-block');
  if (openBtn && window.UIModal) {
    openBtn.addEventListener('click', function () {
      var tpl = document.getElementById('tpl-create-block');
      var wrap = document.createElement('div');
      wrap.appendChild(tpl.content.cloneNode(true));
      UIModal.open({
        title: 'Create a block',
        subtitle: 'Name the section, assign a teacher, and choose college + program.',
        wide: true,
        html: wrap.innerHTML,
        onOpen: function (body) {
          var form = body.querySelector('#create-block-form') || body.querySelector('form');
          if (!form) return;
          var dept = form.querySelector('select[name="department_id"]');
          var course = form.querySelector('select[name="course_id"]');
          if (!dept || !course) return;
