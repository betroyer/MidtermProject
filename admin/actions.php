...
function collect_user_input(array $in, array &$errors): array
{
    ...
    if (!valid_middle_initial($mi)) $errors[] = 'Middle initial must be letters only (optional).';
    ...
    $mi = strtoupper(rtrim($mi, '.'));

    return compact('first', 'last', 'mi', 'email', 'phone', 'age', 'sid', 'address');
}

/** Student enrollment profile: address + emergency contact. */
function collect_student_enrollment_input(array $in, array &$errors): array
{
    $address = trim($in['address'] ?? '');
    $emergency_name = trim($in['emergency_name'] ?? '');
    $emergency_relation = strtolower(trim($in['emergency_relation'] ?? ''));
    $emergency_address = trim($in['emergency_address'] ?? '');
    $emergency_phone = trim($in['emergency_phone'] ?? '');

    if (!valid_address($address)) {
        $errors[] = 'Student address is required.';
    }
    if (!valid_name($emergency_name)) {
        $errors[] = 'Emergency contact name must contain letters only.';
    }
    if (!valid_emergency_relation($emergency_relation)) {
        $errors[] = 'Please select emergency contact type (Parent, Grandparent, or Guardian).';
    }
    if (!valid_address($emergency_address)) {
        $errors[] = 'Emergency contact address is required.';
    }
    if (!valid_phone($emergency_phone)) {
        $errors[] = 'Emergency contact phone must be exactly 11 digits.';
    }

    return compact('address', 'emergency_name', 'emergency_relation', 'emergency_address', 'emergency_phone');
}
...
if ($action === 'add_teacher' || $action === 'add_student') {
    ...
    if ($role === 'student') {
        ...
        if ($year_level < 1 || $year_level > 4) {
            $errors[] = 'Please select year level (1st–4th Year).';
        }
        // 1st year: School ID is unknown — auto-generate (also becomes password).
        if ($year_level === 1) {
            $_POST['school_id'] = generate_next_school_id($pdo);
            $autoSchoolId = true;
        }
        if ($program_id > 0) {
            ...
            if (!$chk->fetchColumn()) {
                ...
                try {
                    $assigned = assign_open_block_for_program($pdo, $program_id);
                    $block_id = (int)$assigned['id'];
                    $assignedBlockName = (string)$assigned['name'];
                } catch (Throwable $e) {
                    $errors[] = $e->getMessage();
                }
            }
        }
        $enroll = collect_student_enrollment_input($_POST, $errors);
    } else {
        $department_id = (int)($_POST['department_id'] ?? 0);
        if ($department_id <= 0) {
            $errors[] = 'Please select the teacher’s department.';
        } else {
            $chk = $pdo->prepare('SELECT id FROM departments WHERE id = :id AND is_active = 1');
            $chk->execute([':id' => $department_id]);
            if (!$chk->fetchColumn()) {
    ...
    $stmt = $pdo->prepare(
        'INSERT INTO users (role, username, password_hash, school_id, first_name, last_name, middle_initial,
                            email, phone, age, address, emergency_name, emergency_relation, emergency_address,
                            emergency_phone, block_id, program_id, year_level, department_id)
         VALUES (:role,:u,:h,:sid,:f,:l,:mi,:e,:p,:a,:addr,:en,:er,:ea,:ep,:b,:prog,:yr,:dept)'
    );
    $stmt->execute([
        ':role' => $role, ':u' => $username, ':h' => $hash, ':sid' => $d['sid'],
        ':f' => $d['first'], ':l' => $d['last'], ':mi' => $d['mi'],
        ':e' => $d['email'], ':p' => $d['phone'], ':a' => (int)$d['age'],
        ':addr' => $enroll['address'] ?: ($d['address'] ?? ''),
        ':en' => $enroll['emergency_name'],
        ':er' => $enroll['emergency_relation'],
        ':ea' => $enroll['emergency_address'],
        ':ep' => $enroll['emergency_phone'],
        ':b' => $block_id, ':prog' => $program_id, ':yr' => $year_level, ':dept' => $department_id,
    ]);
    $newId = (int)$pdo->lastInsertId();

    $extra = '';
    if ($role === 'student' && $block_id > 0 && $program_id > 0 && $year_level >= 1) {
        $r = enroll_student_year_curriculum($pdo, $newId, $block_id, $program_id, $year_level);
        $extra = sprintf(
            ' Year %d subjects applied (%d new enrollments). Class schedule slots created/used: %d. Student can view My Schedule.',
            $year_level,
            $r['enrolled'],
            $r['schedule']
        );
        audit_log(
            'STUDENT_ENROLLED',
            'student#' . $newId,
            $username . '; year=' . $year_level . '; subjects=' . $r['enrolled'] . '; sched=' . $r['schedule']
        );
    }
    ...
    $credMsg = $autoSchoolId
        ? " School ID (also password) auto-generated: {$d['sid']}."
        : ' (password = school ID).';
    $blockMsg = ($role === 'student' && $assignedBlockName !== '')
        ? " Assigned to block {$assignedBlockName} (max " . block_student_capacity() . " students per block)."
        : '';
    set_flash('ok', ucfirst($role) . " account created — username: {$username}.{$credMsg}{$blockMsg}" . $extra);
    redirect($role === 'teacher' ? 'teachers.php' : 'student_view.php?id=' . $newId);
}
...
if ($action === 'update_student') {
    ...
    $exists = $pdo->prepare(
        'SELECT id, username, block_id, program_id, year_level FROM users WHERE id = :id AND role = "student"'
    );
    ...
    $blockNote = '';
    if (!$errors) {
        $needNewBlock = $block_id <= 0 || (int)($row['program_id'] ?? 0) !== $program_id;
        if (!$needNewBlock && block_active_student_count($pdo, $block_id, $id) >= block_student_capacity()) {
            $needNewBlock = true;
        }
        if ($needNewBlock) {
            try {
                $assigned = assign_open_block_for_program($pdo, $program_id, $id);
                $block_id = (int)$assigned['id'];
                $blockNote = ' Block set to ' . $assigned['name'] . '.';
            } catch (Throwable $e) {
                $errors[] = $e->getMessage();
            }
        }
    ...
    $mi = strtoupper(rtrim($mi, '.'));
    $params = [
        ':f' => $first, ':l' => $last, ':mi' => $mi, ':e' => $email, ':p' => $phone,
        ':a' => (int)$age, ':addr' => $enroll['address'],
        ':en' => $enroll['emergency_name'], ':er' => $enroll['emergency_relation'],
        ':ea' => $enroll['emergency_address'], ':ep' => $enroll['emergency_phone'],
        ':b' => $block_id, ':prog' => $program_id, ':yr' => $year_level, ':id' => $id,
    ];

    if ($sid !== '') {
        $stmt = $pdo->prepare(
            'UPDATE users SET first_name=:f, last_name=:l, middle_initial=:mi, email=:e, phone=:p, age=:a,
                    address=:addr, emergency_name=:en, emergency_relation=:er, emergency_address=:ea, emergency_phone=:ep,
                    block_id=:b, program_id=:prog, year_level=:yr, password_hash=:h, school_id=:sid
             WHERE id=:id AND role="student"'
        );
        $params[':h'] = password_hash($sid, PASSWORD_BCRYPT);
        $params[':sid'] = $sid;
        $stmt->execute($params);
    } else {
        $stmt = $pdo->prepare(
            'UPDATE users SET first_name=:f, last_name=:l, middle_initial=:mi, email=:e, phone=:p, age=:a,
                    address=:addr, emergency_name=:en, emergency_relation=:er, emergency_address=:ea, emergency_phone=:ep,
                    block_id=:b, program_id=:prog, year_level=:yr
             WHERE id=:id AND role="student"'
        );
        $stmt->execute($params);
    }

    $changed = (int)$row['block_id'] !== $block_id
        || (int)($row['program_id'] ?? 0) !== $program_id
        || (int)($row['year_level'] ?? 0) !== $year_level;
    if ($changed) {
        enroll_student_year_curriculum($pdo, $id, $block_id, $program_id, $year_level);
    }

    audit_log('STUDENT_UPDATED', 'student#' . $id, $row['username'] . ($sid !== '' ? ' (password reset)' : ''));
    set_flash('ok', 'Student record updated.' . ($changed ? ' Year subjects / schedule refreshed.' : '') . $blockNote);
...
if ($action === 'update_teacher') {
    ...
    if (!valid_phone($phone)) $errors[] = 'Phone must be exactly 11 digits (numbers only).';
    if (!valid_age($age))     $errors[] = 'Age must be a whole number.';
    if ($department_id <= 0)  $errors[] = 'Please select the teacher’s department.';
    if ($sid !== '' && !valid_school_id($sid)) {
        $errors[] = 'School ID / password must look like 2026-00200.';
    }
    if ($department_id > 0) {
    ...
    if ($errors) {
        set_flash('error', implode(' ', $errors));
        $return = trim($_POST['return_to'] ?? '');
        if ($return === 'teachers.php') {
    ...
    if ($sid !== '') {
        $stmt = $pdo->prepare(
            'UPDATE users SET first_name=:f, last_name=:l, email=:e, phone=:p, age=:a, department_id=:d,
                    password_hash=:h, school_id=:sid
             WHERE id=:id AND role="teacher"'
        );
        $stmt->execute([
            ':f' => $first, ':l' => $last, ':e' => $email, ':p' => $phone,
            ':a' => (int)$age, ':d' => $department_id,
            ':h' => password_hash($sid, PASSWORD_BCRYPT), ':sid' => $sid, ':id' => $id,
        ]);
    } else {
        $stmt = $pdo->prepare(
            'UPDATE users SET first_name=:f, last_name=:l, email=:e, phone=:p, age=:a, department_id=:d
        ...
        $stmt->execute([
            ':f' => $first, ':l' => $last, ':e' => $email, ':p' => $phone,
            ':a' => (int)$age, ':d' => $department_id, ':id' => $id,
        ]);
    }
    ...
    $return = trim($_POST['return_to'] ?? '');
    redirect($return === 'teachers.php' ? 'teachers.php' : ('teacher_view.php?id=' . $id));
}
...
/** Safe return path for enrollment forms (default: student detail). */
function enrollment_return_to(int $studentId = 0): string
{
    $raw = trim((string)($_POST['return_to'] ?? ''));
    if ($raw === 'enrollments') {
        $qs = [];
        $block = (int)($_POST['filter_block_id'] ?? 0);
        $offering = (int)($_POST['filter_offering_id'] ?? 0);
        if ($block > 0) {
            $qs['block_id'] = $block;
        }
        if ($offering > 0) {
            $qs['offering_id'] = $offering;
        }
        return 'enrollments.php' . ($qs ? '?' . http_build_query($qs) : '');
    }
    if ($studentId > 0) {
        return 'student_view.php?id=' . $studentId;
    }
    return 'enrollments.php';
}
...
if ($action === 'enroll_student') {
    ...
    $back = enrollment_return_to($studentId);
...
if ($action === 'bulk_enroll_block') {
    require_admin_perm('students');
    ensure_academics_schema($pdo);
    $blockId = (int)($_POST['block_id'] ?? 0);
    $offeringIds = array_values(array_unique(array_filter(
        array_map('intval', (array)($_POST['offering_ids'] ?? [])),
        static fn(int $id): bool => $id > 0
    )));
    $back = enrollment_return_to(0);
    if ($blockId <= 0 || !$offeringIds) {
        set_flash('error', 'Choose a block and at least one subject offering.');
        redirect($back);
    }
    $chk = $pdo->prepare('SELECT id, name FROM blocks WHERE id = :id');
    $chk->execute([':id' => $blockId]);
    $block = $chk->fetch();
    if (!$block) {
        set_flash('error', 'Block not found.');
        redirect($back);
    }

    $placeholders = implode(',', array_fill(0, count($offeringIds), '?'));
    $offStmt = $pdo->prepare(
        "SELECT id FROM subject_offerings WHERE is_active = 1 AND id IN ($placeholders)"
    );
    $offStmt->execute($offeringIds);
    $validOfferings = array_map('intval', $offStmt->fetchAll(PDO::FETCH_COLUMN));
    if (!$validOfferings) {
        set_flash('error', 'No valid active offerings selected.');
        redirect($back);
    }

    $studStmt = $pdo->prepare(
        'SELECT id FROM users WHERE role = "student" AND block_id = :b AND COALESCE(is_active, 1) = 1'
    );
    $studStmt->execute([':b' => $blockId]);
    $studentIds = array_map('intval', $studStmt->fetchAll(PDO::FETCH_COLUMN));
    if (!$studentIds) {
        set_flash('error', 'No active students in this block.');
        redirect($back);
    }
    ...
    foreach ($studentIds as $sid) {
        foreach ($validOfferings as $oid) {
            $ins->execute([':s' => $sid, ':o' => $oid]);
            if ($ins->rowCount() > 0) {
                $created++;
            } else {
                $skipped++;
    ...
    $nStudents = count($studentIds);
    $nOfferings = count($validOfferings);
    audit_log(
        'ENROLLMENT_BULK',
        'block#' . $blockId,
        sprintf(
            '%s: %d student(s) × %d offering(s); created=%d skipped=%d',
            $block['name'],
            $nStudents,
            $nOfferings,
            $created,
    ...
    set_flash(
        'ok',
        sprintf(
            'Enrolled %d student(s) into %d offering(s) (%d new, %d already enrolled).',
            $nStudents,
            $nOfferings,
            $created,
            $skipped
